...
| **Scrape Ladders** | `npm run scrape-ladders` | Pulls pointscore/ladder rows from each active page into `temp/ladders.json` (used by `generate-season-json`) |

### Weekly Fixture Site Generators

The site at [weekly-fixture/](./weekly-fixture/) reads pre-built JSON files committed to the repo. The SPA itself loads only `season.json`; the others are supporting inputs or back-compat artefacts. See [weekly-fixture/README.md](./weekly-fixture/README.md) for the front-end, data shapes, hosting, and PWA details.

| Script | Command | Output |
|--------|---------|--------|
| **Scrape Scores** | `npm run scrape-scores` | `temp/scores.json` — per-match scores scraped from active HV team pages (input to `season.json`) |
| **Scrape Ladders** | `npm run scrape-ladders` | `temp/ladders.json` — pointscore rows scraped from active HV pages (input to `season.json`) |
| **Season JSON** | `npm run generate-season-json` | `weekly-fixture/data/season.json` — **primary SPA source**; joins `temp/scores.json` + `temp/ladders.json` with `config/competitions.json` |
| **Fixtures JSON** | `npm run generate-fixtures-json` | `weekly-fixture/data/fixtures.json` — legacy/back-compat week view (fetches the four category Google Calendar iCal feeds server-side); no longer read by the SPA |
...
The two settings you must set before first use are `clubName` and `calendarPrefix`; the file also stores generated calendar metadata (`clubCalendar`, `categoryCalendars`, `calendarsHomepage`) that the scripts fill in as you go. See the [Setup Guide](./docs/setup.md#1-configuration-setup) for the full field-by-field reference — it is the single source of truth for `settings.json`.

- **clubName**: Used by the competition scraper to find your club in Hockey Victoria competitions. Must match your club name as it appears on the Hockey Victoria website (e.g. `"Footscray"`).
- **calendarPrefix**: Prepended to all Google Calendar names (e.g. `"FHC "` → "FHC Men's Pennant A Grade"). Keep the trailing space if you want one.
...
- Ensure `service-account-key.json` exists in project root, the Google Calendar API is enabled, and the service account has the necessary permissions
- Full walkthrough and checklist: [Setup Guide → Troubleshooting Authentication Errors](./docs/setup.md#troubleshooting-authentication-errors)
...
The [sync-calendars.yml](./.github/workflows/sync-calendars.yml) workflow runs every night at 2 AM Melbourne time. Each step feeds the next; the workflow commits the regenerated JSON + markdown back to `main` and purges the jsDelivr cache so the [weekly-fixture site](./weekly-fixture/) picks up fresh data. Data-only commits do **not** trigger a Netlify redeploy — see [weekly-fixture/README.md → Hosting and data flow](./weekly-fixture/README.md#hosting-and-data-flow) for the Netlify/jsDelivr details.
...
graph TD
    ...
    D --> E[scrape-scores]
    D --> F[scrape-ladders]
    E -->|temp/scores.json| G[generate-season-json]
    F -->|temp/ladders.json| G
    B --> H[generate-fixtures-json]
    H --> I[weekly-fixture/data/fixtures.json]
    G --> J[weekly-fixture/data/season.json]
    D --> K[generate-docs]
    K --> L[docs/competitions.md, docs/competitions-mobile.md]
    K --> M[weekly-fixture/data/subscribe-data.json]
    I --> N[Commit + push]
    J --> N
    L --> N
    M --> N
    N --> O[Purge jsDelivr cache]
```
...
| `scrape-ladders` | Scrapes pointscore rows from active pages into `temp/ladders.json` |
| `generate-season-json` | Joins `temp/scores.json` + `temp/ladders.json` + `config/competitions.json` → `weekly-fixture/data/season.json` (**primary SPA source**) |
| `generate-fixtures-json` | Reads the four category calendars from Google → `weekly-fixture/data/fixtures.json` (legacy/back-compat week view) |
| `generate-docs` | Writes the two markdown calendar indexes plus `weekly-fixture/data/subscribe-data.json` |
| Commit + purge | The bot commits the updated JSON + markdown, then purges jsDelivr so the site serves fresh data |
