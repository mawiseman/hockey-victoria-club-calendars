...
The site at [weekly-fixture/](./weekly-fixture/) reads pre-built JSON files committed to the repo. Each is regenerated by its own script:
...
The [sync-calendars.yml](./.github/workflows/sync-calendars.yml) workflow runs every night at 2 AM Melbourne time and chains six steps. Each one feeds the next; the final step commits everything back to `main`, which triggers a Netlify redeploy of the [weekly-fixture site](./weekly-fixture/).
